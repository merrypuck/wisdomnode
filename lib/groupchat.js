...
newChatroom : function(session) {
	...
	checkSanity: function(message) {
		if (message.indexOf("<script") >= 0 ||
			message.indexOf("<input") >= 0 || 
			message.indexOf("<img") >= 0 ||
			message.indexOf("<javacsript") >= 0 ||
			message.indexOf("<html") >= 0 ||
			message.indexOf("<iframe") >= 0) {
			console.log("special char in chat**********");
			return false;
		} else {
			return true;
		}
	...
	updateChat : function(data) {
		...
		// data.timestamp
		console.log(data);
		if (!this.checkSanity(data.payload)) {
			return;
		}
