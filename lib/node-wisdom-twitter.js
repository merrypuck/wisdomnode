...
wtwitter = {
  ...
  subscribe : function(socket) {
    ...
    socket.emit(this.NEW_SOCIAL_STREAM, this.tweetsLog);
  },

  _add_to_logs : function(tweet) {
    var length = this.tweetsLog.length;
    if (length < 15) {
      this.tweetsLog.push(tweet);
    } else {
      var prevArr = this.tweetsLog.slice(1,15);
      prevArr.push(tweet);
      this.tweetsLog = prevArr;
    }
  },
