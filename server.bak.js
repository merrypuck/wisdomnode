...
var TWITTER_CONSUMER_KEY = "1koAoPkW7asbqbmAEuKuzQ";
var TWITTER_CONSUMER_SECRET = "UpT2G6HHPMXcbCCYfJn6NBUsjbKqtnqG7HxcBS20";
...
passport.use(new TwitterStrategy({
	...
	callbackURL: serverAddr + ":" + port+ "/auth/twitter/callback",
...
app.get('/auth/twitter/callback', 
	...
	function(req, res) {
		...
		var overify = req.query.oauth_verifier;
		req.user.oauth_token = otoken;
		req.user.oauth_verify = overify;
...
app.get('/expert1', function(req, res) {
	...
	tweeter = new OAuth(
		...
		"1.0",
		"http://localhost:8002/auth/twitter/callback",
		'HMAC-SHA1'
		);


	tweeter.post("https://api.twitter.com/1.1/statuses/update.json",
		twitterUsers[req.user.id].token, twitterUsers[req.user.id].tokenSecret, {"status" : 'THIS IS FOR MUKUND @Aalndy'}, "application/json",
...
socket.on('newtweet',function(data){
	console.log("SENDING TWEEEEEET: " + data);
	sendTweet(data);
})
